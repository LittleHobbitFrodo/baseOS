...
fn main() {
    ...
    let (archs, _) = collect_args();


    let (default, specific, vars) = qemu_parameters();

    for arch in archs.iter() {

        noteln!("igniting the OS for {}", arch.triplet().blue());

        let params = parse_params_for(*arch, &default, &specific, &vars);

        debugln!("evaluated parameters: {params:?}");
...
fn qemu_parameters() -> (Params, SpecificParams, Variables) {
