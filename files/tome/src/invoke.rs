...
const FORCE_SWITCH: &'static str = "-reconf";

fn main() {

    let (archs, switches) = collect_args();

    let force = parse_switches(switches);

    if std::fs::exists(PATH.config.tome()).unwrap() {
        //  check if key "configured" is true
        match TomeConfig::load() {
            Ok(cfg) => if cfg.configured {
                //  exit if not forcing
                if force {
                    noteln!("the project is already configured");
                    return
                }
            },
            Err(e) => {
                errorln!("the project configuration is likely corrupted, rewriting configuration");
                errorln!("\t{e:?}");
            }
        }
    }

    noteln!("creating tome configuration");
